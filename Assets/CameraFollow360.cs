...
public class CameraFollow360 : MonoBehaviour {
    ...
    void FixedUpdate () 
	{
		if(player)
		{
			...
			transform.rotation = Quaternion.Slerp(this.transform.rotation, rot, Time.deltaTime * rotSpeed * 0.2f);
			
			Vector3 targetPos = player.transform.position + player.transform.up * height - player.transform.forward * distance;
			
			this.transform.position = Vector3.Lerp(this.transform.position, targetPos, Time.deltaTime * cameraSpeed * 0.2f);
            float acc =player.gameObject.GetComponent<Movementkart>().accelaration;
            this.GetComponent<Camera>().fieldOfView= (acc / 5) + 60f;
